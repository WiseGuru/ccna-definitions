---
{"dg-publish":true,"permalink":"/30-mnemonics-and-summaries/diagnostic-commands/"}
---
...
## `show interfaces`
1. Shows detailed [[15 - OSI Layers/Layer 1\|Layer 1]] and [[15 - OSI Layers/Layer 2\|Layer 2]] information about all interfaces or a specific interface
2. Allows use of the `brief` sub-command to summarize information

## `show ip interface`
1. Shows detailed [[15 - OSI Layers/Layer 3\|Layer 3]] information about all interfaces or a specific interface
2. Can be summarized with the `brief` command
3. Can return IPv6 information with `show ipv6 interface`

## `show ip route`
1. Shows the [[20 - Definitions/Routing Table\|Routing Table]], which detailed *network routes* and how they were learned

## `show cdp neighbors` and `show lldp neighbors`
1. [[20 - Definitions/CDP\|CDP]] is enabled by default on Cisco devices, and communicates identifying information between devices
...
## `show ip protocols`
1. Shows the running [[20 - Definitions/Dynamic Routing Protocols\|Dynamic Routing Protocols]] on a device, and their configurations

## `show arp`
1. Shows the [[20 - Definitions/ARP\|ARP]] entry (i.e., [[20 - Definitions/IPv4\|IP address]] and [[20 - Definitions/MAC Address\|MAC Address]]) for all interfaces


# Network Connectivity
## Ping
1. `ping [destination IP]` sends [[20 - Definitions/ICMP\|ICMP]] echo requests to a destination IP address
2. A quick way to test *two-way [[15 - OSI Layers/Layer 3\|Layer 3]] connectivity* between hosts
	1. The *ICMP echo request* must be able to reach the destination, and the *ICMP echo reply* must be able to return

## Traceroute
1. A step further than Ping, `traceroute [destination IP]` sends [[20 - Definitions/UDP\|UDP]] datagrams with *incrementing [[20 - Definitions/TTL\|TTL]]* values to a destination IP, with a default target *UDP port 33434*
2. Each hop replies, and the route to the destination IP is mapped out.

## Telnet
1. While [[20 - Definitions/Telnet\|Telnet]] is not secure for data transmission, it can be used to test [[15 - OSI Layers/Layer 4\|Layer 4]] connectivity between devices and verify *open ports* with just `telnet [host name or IP] [port number]`
