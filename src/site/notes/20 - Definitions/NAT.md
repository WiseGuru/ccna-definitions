---
{"dg-publish":true,"permalink":"/20-definitions/nat/","tags":["defs_ccna"]}
---

#### NAT
*Network Address Translation (NAT)* is a way to map multiple private IP addresses inside a local network to a public [[20 - Definitions/IPv4\|IP address]] before transferring the information onto the internet.

1. *Static NAT*
	1. Permanent one-to-one mapping between a public and private [[20 - Definitions/IPv4\|IP address]]
...
3. *PAT*
	1. Port Address Translation, also called Dynamic NAT with Overload, is a type of dynamic NAT that bands several local IP addresses to a singular public one using [[15 - OSI Layers/Layer 4\|Layer 4]] ports
	2. It can use one or more public IP addresses
		1. The first public [[20 - Definitions/IPv4\|IP address]] is assigned like normal in Dynamic NAT
		2. The last public [[20 - Definitions/IPv4\|IP address]] is translated using port numbers
...
2.  [[20 - Definitions/NAT\|Static NAT]] configuration
	1.  `Config# ip nat inside source (static/dynamic) (internal IP) (outbound, external IP)`
3.  [[20 - Definitions/NAT\|Dynamic NAT]] configuration
	...
	3.  Associate the ACL with the NAT pool to complete the config
		...
		2. `overload` enables [[20 - Definitions/NAT\|PAT]]
4.  [[20 - Definitions/NAT\|PAT]] with DHCP
...
6.  Verification
	1.  `# sho ip nat translation`
		...
		2.  Inside local address
			1.  The [[20 - Definitions/IPv4\|IP address]] actually configured on the inside host's OS
		...
		4.  Outside local address
			1.  The [[20 - Definitions/IPv4\|IP address]] of the outside host as it appears to the inside network
		5.  Outside global address
			1.  The [[20 - Definitions/IPv4\|IP address]] of the outside host assigned by the host's owner
